...
using PerfusionMachine.Protocols.Helper;
using PerfusionMachine.Enums;

namespace PerfusionMachine.Protocols.V485_1
{
    internal class ThemometerFeedbackResolver : IFeedbackResolver
    {
        ...
        private DirectiveResult ParseRunningResultData(byte[] bytes)
        {
            ...
            data.Temperature = DirectiveHelper.Parse2BytesToNumber(bytes.Skip(4).Take(2).ToArray()) / 10;
            data.Addition = DirectiveHelper.Parse2BytesToNumber(bytes.Skip(6).Take(2).ToArray());
